...
(async()=>{
 ...
 try{
  ...
  for(const size of [{width:320,height:568},{width:390,height:844},{width:667,height:375},{width:844,height:390}]){
   ...
   await page.click('#searchButton');await page.waitForTimeout(300);await fits('.search-box',size.width,size.height);await page.click('#closeSearch');
   ...
   await page.click('#musicOpen');await fits('#musicPlayer',size.width,size.height);await page.click('#musicClose');
   await page.locator('#weatherMap').scrollIntoViewIfNeeded();
   assert.equal((await page.locator('#weatherMap').boundingBox()).height,200);
  }
  await page.click('#musicOpen');await page.setViewportSize({width:320,height:568});await page.waitForTimeout(100);await fits('#musicPlayer',320,568);
  assert.deepEqual(errors,[]);
  console.log('PASS: 4 phone sizes, 6 languages, notifications/search/cart/feedback/music panels, compact map, rotation, no JavaScript errors');
